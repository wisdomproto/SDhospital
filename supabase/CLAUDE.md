...
- ⚠️⚠️ **「행마다 EXISTS」 정책을 쓰면 그 EXISTS 가 짚는 컬럼에 인덱스가 있어야 한다.**
  `drug_external_read` 는 약품 **한 행마다** "내 아이 처방에 이 약이 있나"를 묻는데
  `prescription.drug_id` 에 인덱스가 없어서 **499 × 3,644 행**을 매번 훑었다.
  회차 8건 가져오는 데 **2,002ms** 였고, 인덱스 하나 넣으니 **110ms** 가 됐다(`0035`).
  ⚠️ 정책이 틀린 게 아니다 — 정책은 옳고 **인덱스가 없었다.** 새 정책을 쓸 때마다 확인할 것.
