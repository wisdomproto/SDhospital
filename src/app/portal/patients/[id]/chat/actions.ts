...
⚠️ **다만 입원 중이라고 모든 질문을 넘기지는 않는다.** 묻는 것이 무엇인지 보고 가른다.
  ...
  병동의 지금       "오늘 밥 먹었어요?" · "잘 자나요?" · "언제 퇴원해요?"
                    → ⚠️⚠️ **먼저 <기록>의 일일 리포트를 본다.** 우리는 매일 식사·배변을
                      체크해서 보호자에게 보내고 있고, 그날 것이 이미 나갔으면 **그게 답이다.**
                      「병동만 압니다」로 넘기면 **우리가 아는 걸 모른다고 하는 것**이 된다.
                      그날 리포트가 아직 없을 때만 4번(담당 선생님께 확인)으로 간다.
                      ⚠️ 리포트에 없는 것(지금 이 순간의 컨디션·검사 결과)은 여전히 넘긴다.
