...
**AI 채팅은 샘플까지 나갔다** — 슈슈 기록으로 실제 문답이 된다(`ANTHROPIC_API_KEY` 필요,
`node scripts/try-chat.mjs "질문"` 으로 앱 없이 확인). ⚠️ **샘플은 주치의 확인을 건너뛰고 바로 답한다.**
남은 것은 그 선을 세우는 일 — **원장님이 승인한 답변에서만 고르게 하기**와 발송 전 사람 한 번.
그 착수 조건은 원장님의 **"지금 오세요" 목록** 하나 — 답변 32건 전체 검토를 기다리지 않는다.
