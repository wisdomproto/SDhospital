...
- `src/lib/chat/context.ts` — **AI 채팅이 읽는 재료**: 진료 회차 15건(주 증상·원문·코멘트·처방) +
  발송된 검진 + 최근 30일 생활기록 + 먹이는 것을 한 덩어리 글로 만든다. 질문 버튼(`suggestQuestions`)도 여기서 뽑는다.
  ⚠️ **서버에서만 만들고 클라이언트로 안 내려간다** — 진료 원문이 화면으로 가면 보호자 화면에서 뺀 의미가 없다.
  ⚠️ **의료영상은 안 넣는다** — 판독은 요청·승인을 거쳐야 나가는 것이라 채팅이 우회로가 되면 안 된다.
  화면은 `portal/patients/[id]/chat/` (`actions.ts` 에 SYSTEM 프롬프트 = 문 0~5 분류 + 지켜야 할 선, `ChatBox.tsx`).
  ⚠️ **마크다운을 쓰지 말라고 지시한다** — 말풍선이 평문이라 `#`·`**` 가 기호로 노출된다.
  ⚠️ **샘플이라 주치의 확인 없이 바로 답하고, 호출 횟수 제한이 없다.**
- `VetAnswers.tsx` — 사람에게 넘긴 질문과 그 답. **답이 붙는 순간 알림창이 뜬다**(`.answer-toast`).
...
- `chiefComplaintWarning` (`lib/owner-report.ts`) — 주 증상이 보호자용이 아닌 것 같으면
  **발송 화면에서 경고**한다(막지는 않는다). EMR 에서 옮겨 온 회차에 「검사자료(더케이동물병원)」·
  「안락사」·「치료비 정산」이 그대로 들어 있고, 그 값이 **리포트 제목**이 된다.
  ⚠️ 같은 이유로 **채팅 질문 버튼은 주 증상을 인용하지 않는다**(날짜만) — 화이트리스트를 짜 봤더니
  69명 중 12명이 통과했고 그 안에 「안락사」가 있었다. `suggestQuestions` 는 **떠난 아이면 다른 목록**을 준다.
- `src/lib/chat/scenario.ts` — **시나리오 테스트**(채팅 화면 위 점선 박스, `?s=before|during|after|now`).
  샘플 데이터가 전부 과거라 지금 입원 중인 아이가 사실상 없다 — 그래서 **가짜 입원을 만들지 않고
  「오늘」을 그 아이가 실제로 입원해 있던 날로 옮긴다.** 그러면 그날의 기록이 전부 진짜다.
  `buildPatientContext(…, asOf)` 가 **기준일 이후를 전부 잘라낸다**(안 자르면 채팅이 미래를 알고 답한다).
  ⚠️ 입원 중 판정은 `status` 컬럼이 아니라 **날짜로** 한다 — 컬럼을 믿으면 기준일을 옮겨도 안 바뀐다.
  ⚠️ **보호자에게 앱을 줄 때 `page.tsx` 의 `scenario-box` 블록을 지운다.**
  ⚠️ 떠난 아이는 시나리오를 안 쓴다 — `patient.note` 의 사망 사실이 날짜로 안 걸러져서
  입원 중 시점으로 옮겨도 「이미 떠났다」고 말한다.
