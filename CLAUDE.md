...
**AI 채팅은 실제 환자 69명의 기록 위에서 돈다** (`ANTHROPIC_API_KEY` 필요,
`node scripts/try-chat.mjs "질문"` — 분류까지 같이 찍는다). 세운 선 둘:
① **떠난 아이에게 살아 있는 것처럼 말하지 않는다** (사망 6명, `patient.note`)
② **모르면 지어내지 않고 사람에게 넘긴다** — `ask_vet` 이 「오늘 할 일」 맨 위로 뜨고,
원장님이 쓴 답이 그대로 보호자 채팅에 붙는다. **응급은 넘기지 않고 즉시 전화**(검증됨).

⚠️ **진료기록 추출이 69/106 에서 멈춰 있다** — 남은 37명·1,022장. 이어받는 법과
그 과정에서 나온 것은 **see [docs/superpowers/plans/2026-08-13-emr-extraction.md]**.
읽으면서 나온 「그 집의 사정」 55건은 `patient_caution`(직원 전용)에 있고
**채팅은 아직 안 읽는다** — 다 모은 뒤 무엇을 통과시킬지 원장님과 정하기로 했다.
