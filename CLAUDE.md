...
- **보호자 앱은 탭 넷: 병원 소식 · 진료 기록 · AI 채팅 · 내 정보.** 입원은 탭이 아니다 —
...
- **치료 사례는 만들지 않고 링크한다**(`case_story`). 병원 블로그 글에 태그만 달아 같은 문제의 리포트에 붙인다.
  홈페이지 `/Story` 147편을 긁어 넣는다(`supabase/seed/seed_from_website.mjs`).
  **맞는 게 없으면 아무것도 안 붙인다** — 억지로 채우면 광고로 읽혀 리포트 전체의 신뢰가 깎인다.
  ...
  본문 도입부가 온갖 병명을 스쳐 간종양 글이 "심장"으로 걸린다. 못 고른 글은 `active=false` 로 남긴다.
